...
class LinkWidget extends WidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $access = \Drupal::currentUser()->hasPermission('link to any page');
    if (!$access && !$item->isEmpty()) {
      try {
        $access = $item->getUrl()->access();
      }
    ...
    $element['uri'] = [
      ...
      // to them.
      '#default_value' => (!$item->isEmpty() && $access) ? static::getUriAsDisplayableString($item->uri) : NULL,
