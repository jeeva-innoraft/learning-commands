...
class Sql extends QueryPluginBase {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, EntityTypeManagerInterface $entity_type_manager, DateSqlInterface $date_sql, MessengerInterface $messenger, LoggerChannelInterface $logger = NULL) {
    ...
    if (!$logger) {
      @trigger_error('Calling Sql::__construct() with the $logger argument is supported in drupal:9.5.0 and will be required before drupal:10.1.0.', E_USER_DEPRECATED);
    ...
    foreach ($ids as $index => $relationships) {
      foreach ($relationships as $relationship_id => $id) {
        if (isset($entities[$id])) {
          ...
          // database is inconsistent. Drop that row and log an warning.
